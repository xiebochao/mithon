...
function serialFormUpdate(type) {
    ...
    var serial_form_width = 710;
    if (now_height-50 <= serial_form_height) {
        serial_form_height = now_height - 50;
    }
    if (now_width-100 <= serial_form_width) {
        serial_form_width = now_width - 100;
    }
    if (type)
        return [serial_form_width+"px", serial_form_height+"px"];
    else
        return [(now_width - serial_form_width)/2, (now_height - serial_form_height)/2];
}
...
function init() {
  ...
  var container = document.getElementById('content_area');
    ...
    var nav_item_id = ["li_undo", "li_redo", "li_connect", "li_upload", "li_serial_reset", "li_download", "li_serial_read", "li_layer", "li_simulate"];
    ...
    window.addEventListener('resize', onresize, false);
      ...
      var mediaPath = "";
      if (MixlyUrl.BOARD_CONFIG 
        && MixlyUrl.BOARD_CONFIG.hasOwnProperty("ThirdPartyBoard") 
        && MixlyUrl.BOARD_CONFIG["ThirdPartyBoard"].toLowerCase() == "true") {
...
function show_tag(){
    ...
    tag_select('operate_arduino_compile', 'compile');
    ...
    tag_select('simulate_btn', 'simulate');
    tag_select('operate_simulate_btn', 'simulate');
}
