...
pbc.assignD.get('UART')['check_assign'] = function(py2block, node, targets, value) {
    ...
    if(value._astname === "Call" && funcName === "UART" && value.keywords.length === 3)
        return true;
    return false;
}
...
pbc.assignD.get('UART')['create_block'] = function(py2block, node, targets, value){
    ...
    var pinblock = value.keywords[0].arg.v + "=" + value.keywords[0].value.id.v + ", " +value.keywords[1].arg.v + "=" + value.keywords[1].value.id.v;
    var countblock = value.keywords[2].value.n.v;
    //pbc.pinType = null;
...
function converter(py2block, func, args, keywords, starargs, kwargs, node){
    ...
    if(astname === "Call"){
        if(args[0].func._astname == "Name" && py2block.Name_str(args[0].func) === "bytearray"){ //serial.write(str("XXX"))
        ...
        if(args[0].op.name === "Add" && args[0].right._astname === "Str"
            ...
            && py2block.Name_str(args[0].left.func) === "bytearray"
        ){
...
pbc.objectAttrD.get('in_waiting')['UART'] = function (py2block, node, value, attr) {
    return block('serial_any', node.lineno, {
        'mode': value.id.v.replace("uart","")
    }, {});
}