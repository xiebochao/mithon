...
function echarts_init() {
    ...
    echarts_update = setInterval(function () {
        if (serial_port && serial_port.isOpen) {
            var _time_rate = 0; 
            var _data_length = echarts_data[0].data.length;
            if (_data_length > 20) {
                var _old_time = echarts_data[0].data[_data_length - 21].value[0];
                var _now_time = echarts_data[0].data[_data_length - 1].value[0];
...
function getNumber(val) {
    var numArr = val.match(/-?([1-9]\d*(\.\d*)*|0\.[1-9]\d*)/g);
    var numArr1 = [];
    if (numArr) {
