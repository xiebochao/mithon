...
// 刷新select_com串口设备
function refreshSerialList_select_com() {
  // 添加串口名称
  initSerialList(ports=>{
    const $devNames = $('#select_com');
    $devNames.empty();
    _.map(v=>{$devNames.append($(`<option value="${v}">${v}</option>`));
    }, ports);
  });
}

function refreshSerialList_del(com) {
  // 添加串口名称
  initSerialList(ports=>{
    const $devNames = $('#select_com');
    $devNames.empty();
...
// 刷新select_com串口设备
function py_refreshSerialList_select_com(select) {
...
function py_refreshSerialList_del(select, com) {
  var form = layui.form;
  if (select == "cp") {
    // 添加串口名称
    initcp_SerialList(ports=>{
