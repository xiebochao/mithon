	...
	serial_port.open(function (error) {
	  if (error) {
	    ...
	    parser.on('data', function(data) {
	    	serial_receive += data;
	    	var serialNumber = getNumber(data);
	    	if (myChart && serialNumber.length > 0) {
	    		...
	    		if ((echarts_now_time - echarts_old_time) > 1) {
		            ...
		            while (echarts_data.length < serialNumber.length) {
	    			...
	    			for (var i = 0; i < serialNumber.length; i++) {
					    ...
					    */
					    var now_get_data = echarts_data[i].data;
					    now_get_data.push(serialNumber[i]);
					    var now_show_data1 = {
		    				name: '串口数据' + i,
				            type: 'line',
				            showSymbol: false,
					    ...
					    echarts_data[i] = now_show_data1;
	    			}
	    			echarts_x_value.push(echarts_now_time);
